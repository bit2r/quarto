...
로컬 컴퓨터에 설치된 한글 폰트를 찾지 못해 PDF 파일에 한글 제대로 구현하지 못하는 경우 [구글 CJK 폰트](https://github.com/googlefonts/noto-cjk)를 다운로드 받아 로컬 디렉토리에 저장한다. 그리고, `CJKmainfont`를 지정한다. 글꼴을 사용하는 패키지로 `xeCJK` 패키지를 사용한다.
...
상기 작업을 이용한 최종 PDF 파일은 다음에서 확인 가능하다.
